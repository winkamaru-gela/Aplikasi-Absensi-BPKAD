...
function MainContent() {
  // 1. Ambil pendingAbsensi dan fetchAttendanceByRange dari useAppData
  const { 
    appUser, employees, attendance, pendingAbsensi, settings, holidays, loading, 
    handleAppLogin, handleAppLogout, fetchAttendanceByRange 
  } = useAppData();
  ...
  // UPDATE: Hitung pendingCount langsung dari array pendingAbsensi
      ...
      <main id="main-content" className="flex-1 flex flex-col h-full overflow-hidden relative transition-all duration-300 w-full">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 print:p-0 print:overflow-visible bg-gray-50">
            ...
            <Routes>
                ...
                {isManagement ? (
                    <>
                        {/* UPDATE: Kirim pendingAbsensi, bukan attendance lagi */}
                        <Route 
                            path="/verifikasi-absensi" 
                            element={<AdminTerimaAbsensi employees={employees} pendingAbsensi={pendingAbsensi} />} 
                        />
